...
const getElements = () : {| buttonsContainer : HTMLElement, epsButtonsContainer : HTMLElement, paymentFieldsContainer : HTMLElement |} => {
    const buttonsContainer = document.querySelector('#buttons-container');
    const epsButtonsContainer = document.querySelector(`[${ DATA_ATTRIBUTES.FUNDING_SOURCE }="${ FUNDING.EPS }"]`);
...
const slideUpButtons = () => {
    const { buttonsContainer, epsButtonsContainer, paymentFieldsContainer } = getElements();
...
function initPaymentFields({ props, components, payment, serviceData, config } : InitOptions) : PaymentFlowInstance {
    ...
    console.log('fundingSource in payment-fields ++++++++++++++++++++++++++ ', fundingSource);
